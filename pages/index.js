...
export default function BoostCVAI() {
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {steps.map((s, i) => (
            <div key={s} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 11, color: i === step ? "#e8e8e0" : "#444", letterSpacing: 1, display: window?.innerWidth > 600 ? "block" : "none" }}>{s.toUpperCase()}</span>
              {i < steps.length - 1 && <div style={{ width: 20, height: 1, background: i < step ? "#e8ff00" : "#222", transition: "background 0.3s", marginLeft: 4 }} />}
            </div>
          ))}
        </div>
      </header>
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        {step === 0 && (
          <div className="fade-up" style={{ marginBottom: "3rem" }}>
            ...
            <h1 style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: "clamp(3rem, 8vw, 5.5rem)", lineHeight: 0.95, letterSpacing: 2, marginBottom: "1rem" }}>
              Land the job.<br />
              <span style={{ color: "#e8ff00" }}>Not the bin.</span>
            </h1>
            <p style={{ color: "#666", fontSize: 15, lineHeight: 1.7, maxWidth: 500 }}>
              Paste your CV. Drop in the job description. We rewrite everything to match — keywords, tone, structure. Takes 30 seconds.
            </p>
          </div>
        )}
        ...
        {step === 0 && (
          <div className="fade-up" style={{ animationDelay: "0.1s" }}>
            <div style={{ border: "1px solid #1a1a1a", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 11, color: "#e8ff00", letterSpacing: 2, textTransform: "uppercase" }}>Step 01 — Your CV</span>
                <span style={{ fontSize: 11, color: "#444" }}>{cv.length} chars</span>
              </div>
              <textarea
                value={cv}
                onChange={e => setCv(e.target.value)}
                placeholder="Paste your full CV here — work experience, education, skills, everything. The more detail you give, the better the output."
                style={{ width: "100%", minHeight: 280, background: "#0d0d0d", border: "none", padding: "1.5rem", color: "#ccc", fontSize: 13, lineHeight: 1.9, fontFamily: "inherit" }}
              />
              <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid #1a1a1a", display: "flex", justifyContent: "flex-end" }}>
                <button
                  onClick={() => cv.trim() ? setStep(1) : null}
                  style={{ background: cv.trim() ? "#e8ff00" : "#1a1a1a", color: cv.trim() ? "#0a0a0a" : "#444", border: "none", borderRadius: 6, padding: "12px 32px", fontSize: 13, fontWeight: 700, cursor: cv.trim() ? "pointer" : "not-allowed", fontFamily: "inherit", letterSpacing: 2, textTransform: "uppercase", transition: "all 0.2s" }}
                >
                  Next →
                </button>
              </div>
          ...
          <div className="fade-up">
            <div style={{ border: "1px solid #1a1a1a", borderRadius: 12, overflow: "hidden" }}>
              ...
              <textarea
                ...
                placeholder="Paste the full job description here — responsibilities, requirements, about the company. The full thing."
                style={{ width: "100%", minHeight: 240, background: "#0d0d0d", border: "none", padding: "1.5rem", color: "#ccc", fontSize: 13, lineHeight: 1.9, fontFamily: "inherit", borderBottom: "1px solid #1a1a1a" }}
              />
              ...
              <div style={{ padding: "1rem 1.5rem", display: "flex", justifyContent: "space-between" }}>
                ...
                <button onClick={() => jobDesc.trim() ? generate() : null}
                  style={{ background: jobDesc.trim() ? "#e8ff00" : "#1a1a1a", color: jobDesc.trim() ? "#0a0a0a" : "#444", border: "none", borderRadius: 6, padding: "12px 32px", fontSize: 13, fontWeight: 700, cursor: jobDesc.trim() ? "pointer" : "not-allowed", fontFamily: "inherit", letterSpacing: 2, textTransform: "uppercase", transition: "all 0.2s" }}>
                  Generate ✦
                </button>
              </div>
            </div>
        ...
        {step === 2 && result && (
          <div className="fade-up">
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: "2rem" }}>
              ...
              <div>
                <h2 style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: 24, letterSpacing: 2, margin: 0 }}>Application Ready</h2>
                <p style={{ color: "#555", fontSize: 12, letterSpacing: 1 }}>{jobTitle && company ? `${jobTitle.toUpperCase()} AT ${company.toUpperCase()}` : "TAILORED TO YOUR JOB"}</p>
              </div>
            </div>
            ...
            <div style={{ border: "1px solid #1a1a1a", borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
              ...
              <div className="scroll" style={{ maxHeight: 300, overflowY: "auto", padding: "1.5rem", background: "#0d0d0d" }}>
